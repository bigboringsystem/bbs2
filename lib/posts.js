...
const getInternalLinks = function (reply, host) {
  if (!host) {
    return false;
  }
  ...
  const internalLinks = urls.map((r) => {
    let parsed = r.match(internalLinkRe());
    if (!parsed) {
      return false;
    }
...
const setPagination = function (defaultKey, request, next) {
  let key;
  const cs = db.createKeyStream({
    gte: defaultKey,
    limit: 1
  });
...
exports.getRss = function (request, reply) {
  ...
  setPagination('post!', request, function (err, streamOpt) {
    ...
    const rs = db.createReadStream(streamOpt.stream);
    const domain = conf.get('publicDomain');
...
exports.getAllByUser = function (uid, next) {
  ...
  let getFeedPost = function (uid, posts, next) {
    ...
    posts.forEach((post) => {
      const postid = post.key.split('!')[2];

      const fs = db.createReadStream({
        gte: 'post!' + postid,
        lte: 'post!' + postid + '\xff'
      });
    ...
    setPagination('user!' + uid + '!', request, (err, streamOpt) => {
      ...
      rs.pipe(concat((posts) => {
        ...
        if (request.query.reply_to) {
          replyTo = conf.get('publicDomain') + '/post/' + request.query.reply_to;
        }
...
exports.del = function (request, reply) {
  if (request.auth.credentials.uid === request.payload.uid || request.auth.credentials.op) {
    const deleteKeys = function (keys) {
      let len = keys.length;
    ...
    const keyArr = request.params.key.split('!');
    const postid = keyArr[keyArr.length - 1];
...
const getReplyPosts = function (post, next) {
  const streamOpt = {
  ...
  let replies = [];
  const rs = db.createReadStream(streamOpt);
